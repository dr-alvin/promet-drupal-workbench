...
module.exports=function make(config,scenarios,mode,work='/work') {
  ...
  return {
    id:'promet_d11',engine:'playwright',engineOptions:{browser:'chromium',headless:true,chromiumSandbox:config.browserSandbox!==false,args:[...(config.browserSandbox===false?['--no-sandbox']:[]),'--disable-dev-shm-usage'], waitTimeout:Number(config.navigationTimeoutMs||90000), gotoParameters:{waitUntil:'domcontentloaded',timeout:Number(config.navigationTimeoutMs||90000)}},
    asyncCaptureLimit:Number(process.env.AUDIT_ASYNC_CAPTURE_LIMIT||4),asyncCompareLimit:Number(process.env.AUDIT_ASYNC_COMPARE_LIMIT||4),debug:false,debugWindow:false,
    // Antialiasing is the dominant false-positive source across a core upgrade:
    // identical markup re-rasterises with sub-pixel differences. Ignoring it lets
    // thresholds stay tight instead of being loosened to absorb font noise.
    resembleOutputOptions:{ignoreAntialiasing:config.ignoreAntialiasing!==false,errorType:'movement',transparency:0.3},
